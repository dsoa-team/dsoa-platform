...
public class DsoaBundleListener extends BundleTracker {
	 ...
	 * ClassNotFoundException { URL url = bundle.getEntry(EventList.CONFIG);
	 * if(url != null) { EventList list = (EventList)
	 * JAXBContexts.get(EventList.CONFIG).unmarshal(url);
	 * 
	 * //load eventMap for(Event e : list.getEvents()){
	 * eventMap.put(e.getType(), e); }
	 * 
	 * for(Event e : list.getEvents()) { Map<String, Object> eventProperties =
	 * e.getProperties(); if(null != e.getSuperType()){
	 * eventProperties.putAll(eventMap.get(e.getSuperType()).getProperties()); }
	 * 
	 * Set<String> keys = eventProperties.keySet(); Map<String, Object>
	 * registedProperties = new HashMap<String, Object>(eventProperties);
	 * 
	 * for(String key : keys){ try { registedProperties.put(key,
	 * Class.forName((String) eventProperties.get(key))); } catch
	 * (ClassNotFoundException ex ){ registedProperties.put(key,
	 ...
	 * { URL url = bundle.getEntry(ContextModel.CONFIG); if(url != null) {
	 * ContextModel contextModel = (ContextModel)
	 * JAXBContexts.get(ContextModel.CONFIG).unmarshal(url);
	 * logger.fine("Context :");//LOG for(Context c :
	 * contextModel.getContexts()) {
	 * logger.fine(String.format("Context Category: %s", c.getCategory()));//LOG
	 * 
	 * Map<String, String> elements = c.getElements();
	 * 
	 * for(String key : elements.keySet()) { String e = elements.get(key);
	 * logger.fine(String.format("Element id: %s", e)); } }
	 * 
	 * for(ContextMapping cm : contextModel.getContextMappings()) {
	 * logger.fine(String.format("Context ProcessingMapping: %s",
	 * cm.getCategory()));
	 * 
	 * Map<String, String> contextElements = cm.getContextElements();
	 * 
	 * for(String key : contextElements.keySet()) { String ce =
	 * contextElements.get(key); logger.fine(String.format("Event Property: %s",
	 * ce)); } } } }
	 */
