package br.ufpe.cin.dsoa.platform.registry.normalizer;
			...
			for(int j=0; j<slos.size(); j++) {
				...
				if(slos.get(j).getOperation() != null) {
					...
					*/
					...
					//System.out.println("==>" + Double.valueOf(references[i].getProperty(slos.get(j).getAttribute()+"."+slos.get(j).getOperation()).toString()).doubleValue());
					...
					*/
					...
					//System.out.println("==>" + Double.valueOf(references[i].getProperty(slos.get(j).getAttribute()).toString()).doubleValue());
